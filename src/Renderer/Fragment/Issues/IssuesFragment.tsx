...
export class IssuesFragment extends React.Component<Props, State> {
  ...
  componentDidMount() {
    ...
    window.ipc.on(IssueChannels.reloadIssues, () => this.handleReloadIssuesWithUnselectIssue());
    window.ipc.on(IssueChannels.selectNextIssue, () => this.handleSelectNextPrevIssue(1));
    window.ipc.on(IssueChannels.selectNextUnreadIssue, () => this.handleSelectNextPrevIssue(1, true));
    window.ipc.on(IssueChannels.selectPrevIssue, () => this.handleSelectNextPrevIssue(-1));
    window.ipc.on(IssueChannels.selectPrevUnreadIssue, () => this.handleSelectNextPrevIssue(-1, true));
    window.ipc.on(IssueChannels.toggleRead, () => this.handleToggleRead(this.state.selectedIssue));
    window.ipc.on(IssueChannels.toggleArchive, () => this.handleToggleArchive(this.state.selectedIssue));
    window.ipc.on(IssueChannels.toggleMark, () => this.handleToggleMark(this.state.selectedIssue));
    window.ipc.on(IssueChannels.filterToggleUnread, () => this.handleToggleFilter('is:unread'));
    window.ipc.on(IssueChannels.filterToggleOpen, () => this.handleToggleFilter('is:open'));
    window.ipc.on(IssueChannels.filterToggleMark, () => this.handleToggleFilter('is:bookmark'));
    window.ipc.on(IssueChannels.filterToggleAuthor, () => this.handleToggleFilter(`author:${UserPrefRepo.getUser().login}`));
    window.ipc.on(IssueChannels.filterToggleAssignee, () => this.handleToggleFilter(`assignee:${UserPrefRepo.getUser().login}`));
    window.ipc.on(IssueChannels.clearFilter, () => this.handleExecFilterQuery([]));
  }
